...
echo on
b2 -a toolset=msvc > svg_plot_tests_%date%_%tim%.log 2>&1
b2 -a toolset=gcc --build-type=complete  >> svg_plot_tests_%date%_%tim%.log 2>&1
b2 -a toolset=clang-3.1 --build-type=complete  >> svg_plot_tests_%date%_%tim%.log 2>&1
